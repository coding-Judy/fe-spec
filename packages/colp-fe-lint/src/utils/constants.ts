...
// 读取 package.json
//fs.readFileSync(...): 这是一个同步方法，用于读取指定路径的文件。它接收两个参数：文件路径和编码方式，这里使用 'utf8'
const pkg: Record<string, any> = JSON.parse(
  fs.readFileSync(path.join(__dirname, '../../package.json'), 'utf8'), //表示从当前目录向上移动两个级别，然后再进入 package.json 文件，
);
